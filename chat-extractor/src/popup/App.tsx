...
const EXPORT_MIME_TYPES: Record<Exclude<ExportFormat, 'pdf'>, string> = {
  txt: 'text/plain',
  csv: 'text/csv',
    ...
    <div className="min-w-[800px] min-h-[600px] bg-slate-50 text-slate-900">
      ...
      <main className="flex gap-4 p-4">
        <aside className="w-64 bg-white rounded-lg p-4">
          ...
          <div className="mt-6">
            <button
              className="w-full bg-green-600 hover:bg-green-500 text-white p-2 rounded font-medium"
              onClick={() => chrome.tabs.create({ url: chrome.runtime.getURL('results.html') })}
            >
              View Full Results
            </button>
            <button className="w-full mt-2 bg-blue-600 text-white p-2 rounded" onClick={() => handleExport('json')}>
              Export JSON
            </button>
